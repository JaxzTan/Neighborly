...
var DataLoader = class {
  ...
  load(key) {
    ...
    const batch2 = getCurrentBatch(this);
    ...
    if (cacheMap) {
      ...
      if (cachedPromise) {
        const cacheHits = batch2.cacheHits || (batch2.cacheHits = []);
    ...
    batch2.keys.push(key);
    const promise = new Promise((resolve, reject) => {
      batch2.callbacks.push({ resolve, reject });
    });
    if (cacheMap) {
...
${String(values)}`
    ...
    resolveCacheHits(batch2);
    for (let i = 0; i < batch2.callbacks.length; i++) {
      const value = values[i];
      if (value instanceof Error) {
        batch2.callbacks[i].reject(value);
      } else {
        batch2.callbacks[i].resolve(value);
      }
    }
  }).catch((error) => {
...
function failedDispatch(loader, batch2, error) {
  resolveCacheHits(batch2);
  for (let i = 0; i < batch2.keys.length; i++) {
    loader.clear(batch2.keys[i]);
    batch2.callbacks[i].reject(error);
  }
}
function resolveCacheHits(batch2) {
  if (batch2.cacheHits) {
    for (let i = 0; i < batch2.cacheHits.length; i++) {
      batch2.cacheHits[i]();
    }
  }
}
...
var _Transaction = class _Transaction2 {
  ...
  static fromKind(serialized) {
    ...
    __privateSet2(tx, _inputSection, __privateGet2(tx, _data).inputs.slice());
    __privateSet2(tx, _commandSection, __privateGet2(tx, _data).commands.slice());
    __privateSet2(tx, _availableResults, new Set(__privateGet2(tx, _commandSection).map((_, i) => i)));
    return tx;
  }
  /**
  ...
  static from(transaction) {
    ...
    __privateSet2(newTransaction, _inputSection, __privateGet2(newTransaction, _data).inputs.slice());
    __privateSet2(newTransaction, _commandSection, __privateGet2(newTransaction, _data).commands.slice());
    __privateSet2(newTransaction, _availableResults, new Set(__privateGet2(newTransaction, _commandSection).map((_, i) => i)));
    ...
    if (typeof command === "function") {
      ...
      const placeholder = __privateMethod(this, _Transaction_instances, addCommand_fn).call(this, {
        ...
        $Intent: {
          ...
          inputs: {},
          data: {
            resultIndex: __privateGet2(this, _data).commands.length,
            result: null
          }
        }
      });
  ...
  async toJSON(options = {}) {
    ...
    return JSON.stringify(
      parse(
        ...
        fullyResolved ? {
          ...__privateGet2(this, _data).snapshot(),
          digest: __privateGet2(this, _data).getDigest()
        } : __privateGet2(this, _data).snapshot()
      ),
  ...
  isFullyResolved() {
    ...
    if (__privateGet2(this, _pendingPromises).size > 0) {
      return false;
    }
    if (__privateGet2(this, _data).commands.some((cmd) => cmd.$Intent)) {
      return false;
    }
    if (needsTransactionResolution(__privateGet2(this, _data), {})) {
      return false;
    }
    return true;
  }
...
runPlugins_fn = async function(plugins, options) {
  ...
  await createNext(0)();
  __privateSet2(this, _inputSection, __privateGet2(this, _data).inputs.slice());
  __privateSet2(this, _commandSection, __privateGet2(this, _data).commands.slice());
};
...
sortCommandsAndInputs_fn = function() {
  ...
  __privateSet2(this, _availableResults, new Set(filteredCommands.map((_, i) => i)));
...
function getFirstLevelNamedTypes(types) {
  ...
  for (const type of types) {
    findMvrNames(type).forEach((name2) => results.add(name2));
  }
  return results;
}
function findMvrNames(type) {
  ...
  for (const param of tag.typeParams) {
    findMvrNames(param).forEach((name2) => types.add(name2));
  }
  return types;
}
...
function replaceNames(builder, cache) {
  ...
  for (const command of builder.commands) {
    ...
    if (hasMvrName(name2) && !cache.packages[name2])
      throw new Error(`No address found for package: ${name2}`);
    if (hasMvrName(name2)) {
      nameParts[0] = cache.packages[name2];
...
*/
//# sourceMappingURL=chunk-6XQOA6MW.js.map
