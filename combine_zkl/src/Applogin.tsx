...
import type { PiAppleLogoThin } from "react-icons/pi";
...
function toPkIdentifier(account: AccountData): ZkLoginPublicIdentifier {
  ...
  let pk = toZkLoginPublicIdentifier(BigInt(addressSeed), account.iss);
  return pk;
}

function toMultisigAddress(
...
export const App: React.FC = () => {
  ...
  const [msPublicKey, setMsPublicKey] = useState<MultiSigPublicKey | null>(
    null
  ); // multisig pubkey
  ...
  const addPrivateKeyHandler = () => {
    ...
    let privateKey = {
      ...
      publicKey: sk.getPublicKey().toBase64(),
      scheme: sk.getKeyScheme(),
    };
    const updatedPrivateKeys = [...privateKeys, privateKey];
    setPrivateKeys(updatedPrivateKeys);
    sessionStorage.setItem(
      insecurePrivateKeys,
      JSON.stringify(updatedPrivateKeys)
    );
  };
    ...
    <div id="page">
          ...
          {accounts.current.map((acct) => {
            return (
              <div className="account" key={acct.userAddr}>
                ...
                <div>
                  Public Identifier: {toB64(toPkIdentifier(acct).toRawBytes())}
                </div>
              ...
              <div className="account">
                ...
                {msAddress && txBytes && (
                  <button
                    ...
                    disabled={!balance}
                    onClick={() => {
                      signTxWithPrivateKey(k, txBytes);
                    }}
                  >
                    Sign Transaction
                  </button>
      ...
      {(accounts.current.length > 0 || privateKeys.length) && (
        <button
          ...
          onClick={() => {
            let [msPublicKey, msAddress] = toMultisigAddress(
              accounts.current,
              privateKeys,
              threshold
            );
      ...
      {txBytes && msPublicKey && (
        <button
          className={`btn-send ${!balance ? "disabled" : ""}`}
          onClick={() => {
            sendTransaction(txBytes, msPublicKey, sigs);
          }}
        >
          Submit Transaction
        </button>
      ...
      {txDigest && (
        <div>
          <h2>Tx digest:</h2>
          <div>
            https://explorer.polymedia.app/txblock/{txDigest}?network=devnet
          </div>
